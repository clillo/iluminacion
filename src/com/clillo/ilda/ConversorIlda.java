...
public class ConversorIlda {
	
	private IldaFormat ildaFormat = new IldaFormat();
    ...
    private static String getBinario(double d){
    	return (int )d+"";
    	/*
    	String str = Integer.toBinaryString((int)d);
    	if (str.length()<8)
    ...
    public static void main(String args[]) throws Exception {
        ...
        File f = new File("D:\\Laser\\new\\3D2DRudy.ild");
        f = fc.getSelectedFile();
        
        ConversorIlda reader = new ConversorIlda();
        IldaFormat content = reader.read(f);
        ...
        for (CoordinateHeader coordinateHeader : coordinateHeaders) {
            ...
            //	if (coordinateHeader.getFrameNumber()==1){
            		int x = (int) (IldaFormat.MAX_WIDTH/2 + dat.getX()) ;
            		int y = (int) (IldaFormat.MAX_HEIGHT/2 - dat.getY()) ;

    	            sb.append(getBinario(x / IldaFormat.MAX_WIDTH * 255)).append(' ');
    	            sb.append(getBinario(y /IldaFormat.MAX_HEIGHT * 255)).append(' ');
    	            sb.append((dat.getColorData().getCode()!=0?1:0)).append(' ');
    	            sb.append(coordinateHeader.getFrameNumber()).append('\n');
    	            n++;
            		//System.out.println(  getBinario(x / IldaFormat.MAX_WIDTH * 255) + " " + getBinario(y /IldaFormat.MAX_HEIGHT*255) + " " + (dat.getColorData().getCode()!=0?1:0));
            }
        }
    ...
    public IldaFormat read(File fileName) throws IOException, URISyntaxException {
        ...
        while (coordinateHeader.getTotalPoints() > 0) {
            ...
            for (int i = 0; i < coordinateHeader.getTotalPoints(); i++) {
                byte[] data = Arrays.copyOfRange(fileArray, offset + IldaFormat.COORDINATE_HEADER_SIZE + i * IldaFormat.COORDINATE_DATA_SIZE, offset + IldaFormat.COORDINATE_HEADER_SIZE + (i + 1) * IldaFormat.COORDINATE_DATA_SIZE);
        ...
        int start_color_header = offset + IldaFormat.COORDINATE_HEADER_SIZE + (coordinateHeader.getTotalPoints() * IldaFormat.COORDINATE_DATA_SIZE);
        if (start_color_header < fileArray.length) {
            ...
            for (int i = 0; i < colorHeader.getTotalColors(); i++) {
                data = Arrays.copyOfRange(fileArray, start_color_data + i * IldaFormat.COLOR_DATA_SIZE, start_color_data + (i + 1) * IldaFormat.COLOR_DATA_SIZE);
